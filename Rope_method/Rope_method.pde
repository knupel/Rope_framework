...
void setup() {
	println(max(Vec2(23,34),Vec2(45,2)));
}
/*
int c;
void setup() {
...
void draw() {
  ...
  float direction = map(mouseX,0,width,0,1);
  float distance = brightness *10.;
...
void main() {
  ...
  float v1 = noise3d(Vec3(uv.mult(strength), 0.0));
  float v2 = noise3d(Vec3(uv.mult(strength), 1.0));
